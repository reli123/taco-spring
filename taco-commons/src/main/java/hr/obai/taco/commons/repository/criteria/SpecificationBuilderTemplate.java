...
import hr.obai.taco.commons.repository.filter.Filter;
import hr.obai.taco.commons.repository.filter.RangeFilter;
import hr.obai.taco.commons.repository.filter.StringFilter;
...
public abstract class SpecificationBuilderTemplate<ENTITY> {
  /**
  ...
  protected <X extends Comparable<? super X>> Specification<ENTITY> valueBetween(
          Function<Root<ENTITY>, Expression<X>> metaclassFunction, final X value1, final X value2) {
    return (root, query, builder) -> builder.between(metaclassFunction.apply(root), value1, value2);
  }
